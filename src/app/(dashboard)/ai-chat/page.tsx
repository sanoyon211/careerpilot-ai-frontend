    ...
    <div className="flex flex-col h-[calc(100vh-100px)] md:h-[calc(100vh-64px)] -m-4 md:-m-8">
      ...
      <div className="bg-card border-b p-4 flex items-center justify-between shadow-sm z-10 shrink-0">
        <div className="flex items-center gap-3">
          <div className="h-10 w-10 rounded-full bg-rose-600 text-white flex items-center justify-center shadow-md shadow-rose-500/20">
            <Sparkles className="h-5 w-5" />
          </div>
          <div>
            <h1 className="font-extrabold text-lg leading-tight">{mode === 'coach' ? 'Career Coach AI' : 'Mock Interviewer'}</h1>
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center bg-muted/60 p-1 rounded-full border">
            <button
              onClick={() => {
                setMode("coach");
                setMessages(INITIAL_COACH_MESSAGES);
              }}
              className={`px-4 py-1.5 text-xs font-extrabold rounded-full transition-colors ${mode === 'coach' ? 'bg-rose-600 text-white shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}
            >
              Coach
            </button>
            <button
              ...
              className={`px-4 py-1.5 text-xs font-extrabold rounded-full transition-colors ${mode === 'mock-interview' ? 'bg-rose-600 text-white shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}
            >
              Mock Interview
            </button>
          </div>
          <button onClick={() => setMessages(mode === 'coach' ? INITIAL_COACH_MESSAGES : INITIAL_MOCK_MESSAGES)} className="text-muted-foreground hover:text-rose-600 transition-colors p-2" title="Clear Chat">
